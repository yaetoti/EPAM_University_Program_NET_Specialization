...
namespace OuterJoin.Tests
{
    ...
    public class SqlTaskTests
    {
        ...
        [Test]
        public void SelectQuery_ContainsWhereIsNull([Range(1, FilesCount)] int index)
        {
            var actual = Queries[index - 1];
            Assert.IsTrue(SelectHelper.ContainsWhere(actual), "Query should contain 'WHERE' statements with 'IS NULL' operator.");
        }
        ...
        [Test]
        public void SelectQuery_ContainsLeftJoin([Range(1, FilesCount)] int index)
        {
            var actual = Queries[index - 1];
            Assert.IsTrue(SelectHelper.ContainsLeftJoin(actual), "Query should contain 'LEFT JOIN' statement.");
        }

        [TestCase(3)]
        public void SelectQuery_ContainsUnion(int index)
        {
            var actual = Queries[index - 1];
            Assert.IsTrue(SelectHelper.ContainsUnion(actual), "Query should contain 'UNION' statement.");
        }
